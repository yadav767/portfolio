...
const AdminProject = () => {
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (selectedItemForEdit) {
                response = await axios.put("https://portfolio-green-ten-61.vercel.app/api/portfolio/update-project", { ...values, _id: selectedItemForEdit._id })
            } else {
                response = await axios.post("https://portfolio-green-ten-61.vercel.app/api/portfolio/add-project", values)
            }
    ...
    const onDelete = async (item) => {
        try {
            dispatch(setLoading())
            const response = await axios.post("https://portfolio-green-ten-61.vercel.app/api/portfolio/delete-project", { _id: item._id })
            dispatch(hideLoading())
            if (response.data.success) {
                message.success(response.data.message)
                dispatch(hideLoading())
